...
router.get('/stats', async (req, res) => {
  try {
    const data = await arcOrFallback(
      req,
      async (key) => {
        // Compose a stats summary from rounds + profile when ArcTracker is linked.
        const { summarizeRounds } = await import('../services/arctracker.js');
        const [profile, summary] = await Promise.all([
          ArcTrackerAPI.getProfile(key).catch(() => null),
          summarizeRounds(key, { limit: 5000 }).catch(() => null),
        ]);
        return { profile, ...(summary || {}) };
      },
      () => EmbarkAPI.getStats(req.user.id),
    );
    res.json(data);
  } catch (err) {
