      ...
      if (atKey) {
        const [profile, summary, stashRes, embarkProfile, embarkProgression] =
          await Promise.all([
            ...
            safe(UserDataAPI.getStats(req.user.id)),
  ...
  try {
    ...
    if (atKey) {
      const [
        ...
        safe(UserDataAPI.getMapPerformance(req.user.id)),
      ]);
      stats = summary
        ? {
      ...
      weaponKills = weaponKillsRes;
      enemyKills = enemyKillsRes;
      mapPerformance = mapPerformanceRes;
...
// Primary: dedicated ArcTracker /api/v2/user/map-performance endpoint (pre-aggregated).
// Returns: { maps: [{ mapTargetId, mapName, raids, extracted, totalDurationMs, totalNetValue,
//             survivalRate, avgDurationMs, avgNetValue }],
//           source: "arctracker" | "aggregated" }
router.get('/map-performance', async (req, res) => {
  try {
    // Try the dedicated ArcTracker endpoint first — much cheaper than full round scan.
    const dedicated = await UserDataAPI.getMapPerformance(req.user.id).catch(
      () => null,
    );
    if (
      dedicated?.maps &&
      Array.isArray(dedicated.maps) &&
      dedicated.maps.length > 0
    ) {
      const maps = dedicated.maps.map((m) => ({
        mapTargetId:
          m.mapTargetId ||
          m.mapId ||
          m.map_id ||
        ...
        mapId:
          ...
          m.id ||
          null,
        mapName: m.mapName || m.map_name || m.map || 'Unknown',
        raids: m.raids || 0,
        extracted: m.extracted || 0,
        totalDurationMs: m.totalDurationMs || 0,
        totalNetValue: m.totalNetValue || 0,
        survivalRate: m.raids > 0 ? m.extracted / m.raids : 0,
        avgDurationMs:
          m.raids > 0 ? Math.round(m.totalDurationMs / m.raids) : 0,
        avgNetValue: m.raids > 0 ? Math.round(m.totalNetValue / m.raids) : 0,
      }));
      return res.json({ maps, source: 'arctracker' });
    }

    // Fallback: compute from aggregated round history.
    const overview = await buildStatsOverview(req.user.id);
    const mapSpecificData = (
      overview.map_specific_data ||
      overview.maps ||
      []
    ).map((m) => ({
      mapTargetId:
        m.mapTargetId || m.mapId || m.map_id || m.map_target_id || m.id || null,
      mapId:
        m.mapId || m.map_id || m.mapTargetId || m.map_target_id || m.id || null,
      mapName: m.map_name || m.map || 'Unknown',
      raids: m.rounds_played || m.roundsPlayed || 0,
      extracted: m.extractions || 0,
      totalDurationMs: Math.floor(
        (m.time_topside || m.timeTopside || 0) * 1000,
      ),
      totalNetValue: m.net_profit || m.netProfit || 0,
      survivalRate:
        typeof m.survival_rate === 'number'
          ? m.survival_rate
          : m.survivalRate || 0,
      avgDurationMs: 0,
      avgNetValue:
        m.roundsPlayed > 0 ? Math.round(m.netProfit / m.roundsPlayed) : 0,
    }));
    res.json({ maps: mapSpecificData, source: 'aggregated' });
  } catch (err) {
...
// GET /api/player/enemy-kills — Lifetime enemy kill breakdown from the dedicated ArcTracker endpoint.
// Returns: { enemies: [{ targetId, name, count }], source: "arctracker" | "aggregated" }
router.get('/enemy-kills', async (req, res) => {
  try {
    // Try dedicated ArcTracker endpoint first.
    const dedicated = await UserDataAPI.getEnemyKills(req.user.id).catch(
      () => null,
    );
    if (dedicated?.enemies && Array.isArray(dedicated.enemies)) {
      return res.json({ enemies: dedicated.enemies, source: 'arctracker' });
    }
    // Fallback: aggregate from stats overview.
    const overview = await buildStatsOverview(req.user.id);
    const unitBreakdown = overview?.combat_detailed?.unit_breakdown || {};
    const enemies = Object.entries(unitBreakdown).map(([key, count]) => ({
      name: key.replace(/^kills_/, '').replace(/^\w/, (c) => c.toUpperCase()),
      count,
    }));
    res.json({ enemies, source: 'aggregated' });
  } catch (err) {
...
// GET /api/player/weapon-kills — Lifetime weapon kill totals from the dedicated ArcTracker endpoint.
// Returns: { weapons: [{ weaponAssetId, itemId, name, count }], source: "arctracker" | "aggregated" }
router.get('/weapon-kills', async (req, res) => {
  try {
    // Try dedicated ArcTracker endpoint first.
    const dedicated = await UserDataAPI.getWeaponKills(req.user.id).catch(
      () => null,
    );
    if (dedicated?.weapons && Array.isArray(dedicated.weapons)) {
      logger.debug(
        '[Player] Weapon kills returning arctracker data:',
        dedicated.weapons.length,
        'weapons',
      );
      return res.json({ weapons: dedicated.weapons, source: 'arctracker' });
    }
    // Fallback: weaponDamageBreakdown is DAMAGE, not kills:
    //   { weaponName, amount }
    // Do not invent weapon kill counts from damage rows. Return damage-only rows
    // with count/kills set to 0 so the UI can distinguish missing dedicated
    // weapon-kills data from real zero kills.
    const rounds = await UserDataAPI.getRounds(req.user.id).catch(() => []);
    const tally = {};
    for (const r of Array.isArray(rounds) ? rounds : []) {
      if (!Array.isArray(r.weaponDamageBreakdown)) continue;
      for (const w of r.weaponDamageBreakdown) {
        const name = w.weaponName || w.name || '';
        if (!name) continue;
        tally[name] = (tally[name] || 0) + Number(w.amount ?? w.damage ?? 0);
      }
    }
    const weapons = Object.entries(tally)
      .map(([name, damage]) => ({ name, count: 0, kills: 0, damage }))
      .sort((a, b) => b.damage - a.damage);
    logger.debug(
      '[Player] Weapon kills returning aggregated data:',
      weapons.length,
      'weapons',
    );
    res.json({ weapons, source: 'aggregated' });
  } catch (err) {
...
router.get('/telepathy', async (req, res) => {
  try {
    ...
    const [stats, rounds, stash, loadout, mapPerformance] = await Promise.all([
      UserDataAPI.getStats(req.user.id).catch(() => null),
      ...
      UserDataAPI.getMapPerformance(req.user.id).catch(() => null),
    ]);
