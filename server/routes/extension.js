...
router.post('/sync', async (req, res) => {
  try {
    ...
    const normalizedInventory = normalizeInventorySnapshot(payload);
    ...
    const normalizedProgress = normalizeProgressSnapshot(payload);
    ...
    if (payload.rounds || payload.stats) {
    ...
    res.json({
      status: 'synced',
      received: true,
      endpoints: Object.keys(payload).filter((k) => !k.startsWith('_')),
      lastSyncedAt: syncDoc.syncedAt,
      nextSyncAt: new Date(syncDoc.syncedAt.getTime() + 15 * 60 * 1000),
