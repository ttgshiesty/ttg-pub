...
export const ArcTrackerAPI = {
  ...
  getSummary: (userKey, opts = {}) =>
    fetchArcTracker(userKey, '/api/embark/stats/summary', {
      locale: opts.locale || 'en',
    }),
  ...
  getEnemyKills: (userKey, opts = {}) =>
    fetchArcTracker(userKey, '/api/embark/stats/enemy-kills', {
      locale: opts.locale || 'en',
    }),

  getMapPerformance: (userKey, opts = {}) =>
    fetchArcTracker(userKey, '/api/embark/stats/map-performance', {
      locale: opts.locale || 'en',
    }),
  ...
  getWeaponKills: (userKey, opts = {}) =>
    fetchArcTracker(userKey, '/api/embark/stats/weapon-kills', {
      locale: opts.locale || 'en',
    }),

  getStatsRounds: (userKey, opts = {}) =>
    fetchArcTracker(userKey, '/api/embark/stats/rounds', {
      locale: opts.locale || 'en',
  ...
  runDiagnostics: async (userKey) => {
    const endpoints = [
      ...
      '/api/v2/user/enemy-kills?locale=en',
      '/api/embark/stats/map-performance',
      '/api/v2/user/expedition-status?locale=en',
...
export async function summarizeRounds(userKey, opts = {}) {
  // Use getAllRounds to fetch up to 5000 rounds, not just 200
  ...
  let arcKills = 0;
  let playerKills = 0;
  let downs = 0; // MetaForge: "Downs"
  let Downs = 0; // MetaForge: "Downs"
  let damage = 0;
  let damageTaken = 0;
  let total_damageTaken = 0; // guesssssss
  let Damage_Received = 0; // MetaForge: "Damage Received"
  let healthRestored = 0; // MetaForge: "Health Restored / Healed"
  let Health_Restored = 0; // MetaForge: "Health Restored / Healed"
  let Healed = 0; // MetaForge: "Health Restored / Healed"
  let lootValue = 0;
  let loadoutValue = 0;
  let netValue = 0;
  let containersLooted = 0;
  let totalXp = 0; // MetaForge: "Total XP"
  let Total_XP = 0; // MetaForge: "Total XP"
  let topsideSeconds = 0; // MetaForge: "Time Topside"
  let Time_Topside = 0; // MetaForge: "Time Topside"

  // MetaForge: "ARC Enemies Destroyed" — breakdown by enemy type
  const arcEnemyBreakdown = {};

  // MetaForge: "Top Weapons" — total damage by weapon name
  const weaponDamageTotals = {};

  // MetaForge: "Map Performance" — per-map stats
  const mapStats = {};

  for (const r of rounds) {
    totalRaids += 1;
    // Outcome priority: outcome > status
    const status = (r.outcome || r.status || '').toString().toLowerCase();
    if (status === 'extracted' || status.includes('extract')) extractions += 1;
    if (status === 'failed' || status === 'died') deaths += 1;

    // Real field names priority mapping
    const roundArcKills = Number(r.arcKills ?? r.arc_kills ?? 0) || 0;
    const roundPlayerKills = Number(r.playerKills ?? r.player_kills ?? 0) || 0;
    arcKills += roundArcKills;
    playerKills += roundPlayerKills;
    // Total Kills: use provided kills field if available, else sum
    kills += Number(r.kills ?? roundArcKills + roundPlayerKills) || 0;

    // Damage priority: damage > damageDealt
    damage += Number(r.damage ?? r.damageDealt ?? r.damage_dealt ?? 0) || 0;

    // Loot/Net Value priority: valueExtracted > lootValue for extraction, netValue > netProfit for net
    const roundNetValue = Number(r.netValue ?? r.netProfit ?? 0) || 0;
    const roundValueExtracted =
      Number(r.valueExtracted ?? r.lootValue ?? 0) || 0;
    const roundValueBroughtIn =
      Number(r.valueBroughtIn ?? r.loadoutValue ?? 0) || 0;

    lootValue += roundValueExtracted;
    loadoutValue += roundValueBroughtIn;
    netValue += roundNetValue;
    ...
    // MetaForge extended fields
    downs +=
      Number(r.playerDowns ?? r.downs ?? r.timesDown ?? r.downedCount ?? 0) ||
      0;
    damageTaken +=
      Number(
        r.damageTaken ??
    ...
    // Score priority: score > xp
    totalXp +=
      Number(r.score ?? r.xp ?? r.xpEarned ?? r.experience ?? r.exp ?? 0) || 0;

    // Duration priority: durationMs > duration
    topsideSeconds +=
      Number(
        r.durationMs != null
          ? Number(r.durationMs) / 1000
          : (r.duration ?? r.topsideTime ?? r.topside_time ?? 0),
      ) || 0;

    // ARC enemy type breakdown (MetaForge: "ARC Enemies Destroyed")
    if (r.arcEnemyBreakdown && typeof r.arcEnemyBreakdown === 'object') {
      for (const [enemy, count] of Object.entries(r.arcEnemyBreakdown)) {
        arcEnemyBreakdown[enemy] =
          (arcEnemyBreakdown[enemy] || 0) + Number(count || 0);
      }
    }
    // Also handle array format: [{ name, count }]
    if (Array.isArray(r.arcKillBreakdown)) {
      for (const e of r.arcKillBreakdown) {
        const nm = e.name || e.targetName || e.type || e.enemy || '';
        if (nm)
          arcEnemyBreakdown[nm] =
            (arcEnemyBreakdown[nm] || 0) + Number(e.count ?? e.kills ?? 1);
      }
    }

    // Weapon damage breakdown (MetaForge: "Top Weapons")
    if (Array.isArray(r.weaponDamageBreakdown)) {
      for (const w of r.weaponDamageBreakdown) {
        const nm = w.weaponName || w.name || w.weapon || '';
        if (nm)
          weaponDamageTotals[nm] =
            (weaponDamageTotals[nm] || 0) + Number(w.amount ?? w.damage ?? 0);
      }
    }

    // Per-map stats (MetaForge: "Map Performance")
    const mapName = r.mapName || r.map_name || r.map || 'Unknown';
    if (!mapStats[mapName]) {
      mapStats[mapName] = {
        mapName,
        rounds: 0,
    ...
    mapStats[mapName].rounds += 1;
    if (status === 'extracted' || status.includes('extract'))
      mapStats[mapName].extractions += 1;

    mapStats[mapName].netProfit += roundNetValue;

    mapStats[mapName].topsideSeconds +=
      Number(
        r.durationMs != null ? Number(r.durationMs) / 1000 : (r.duration ?? 0),
      ) || 0;
  }

  // Compute derived MetaForge stats
  const survivedRaids = totalRaids - deaths;
  const survivalRate = totalRaids > 0 ? (survivedRaids / totalRaids) * 100 : 0;
  const kdRatio = deaths > 0 ? playerKills / deaths : playerKills;
  const avgProfit = totalRaids > 0 ? Math.round(lootValue / totalRaids) : 0;
  const avgDamagePerRound =
    totalRaids > 0 ? Math.round(damage / totalRaids) : 0;
  const lootEfficiencyPerMin =
    topsideSeconds > 0 ? Math.round(lootValue / (topsideSeconds / 60)) : 0;

  // Top 10 weapons by damage
  const topWeapons = Object.entries(weaponDamageTotals)
    .map(([name, dmg]) => ({ name, damage: dmg }))
    .sort((a, b) => b.damage - a.damage)
    .slice(0, 10);

  // Map performance sorted by raids
  const mapPerformance = Object.values(mapStats)
    .sort((a, b) => b.rounds - a.rounds)
    .map((m) => ({
      ...m,
      survivalRate: m.rounds > 0 ? (m.extractions / m.rounds) * 100 : 0,
      avgProfit: m.rounds > 0 ? Math.round(m.netProfit / m.rounds) : 0,
    }));
  ...
  return {
    ...
    topsideSeconds,
    lootValue,
    valueExtracted: lootValue,
    totalValueExtracted: lootValue,
    loadoutValue,
    valueBroughtIn: loadoutValue,
    totalValueBroughtIn: loadoutValue,
    netValue,
    netProfit: netValue,
