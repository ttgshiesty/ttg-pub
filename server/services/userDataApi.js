...
import { ArcTrackerAPI, summarizeRounds } from './arctracker.js';
...
async function getUnifiedStatsForUser(discordId) {
  ...
  const [summary, calculated, profile, metaStats, extensionStats] =
    await Promise.all([
      key ? ArcTrackerAPI.getSummary(key).catch(() => null) : null,
...
export const UserDataAPI = {
  ...
  getEnemyKills: async (discordId) => {
    ...
    try {
      const key = await getArcKey(discordId);
      if (key) {
        const data = await ArcTrackerAPI.getEnemyKills(key);
        arcData = normalizeMetaForgeEnemies(data) || (data ? { ...data } : null);
        if (arcData) arcData.source = 'arctracker';
        if (arcData) {
      ...
      logger.warn('[UserDataAPI] ArcTracker enemy kills failed:', err.message);
    }
    try {
  ...
  getMapPerformance: async (discordId) => {
    ...
    try {
      const key = await getArcKey(discordId);
      if (key) {
        const data = await ArcTrackerAPI.getMapPerformance(key);
        arcData = data ? { ...data, source: 'arctracker' } : null;
        if (arcData) {
          await saveProviderSnapshot(
            discordId,
            'arctracker',
      ...
      logger.warn('[UserDataAPI] ArcTracker map performance failed:', err.message);
    }
    try {
      ...
      logger.warn('[UserDataAPI] MetaForge map performance failed:', err.message);
    }
    try {
      ...
      logger.warn('[UserDataAPI] Extension map performance failed:', err.message);
    }
    if (
  ...
  getWeaponKills: async (discordId) => {
    ...
    try {
      const key = await getArcKey(discordId);
      if (key) {
        const data = await ArcTrackerAPI.getWeaponKills(key);
      ...
      logger.warn('[UserDataAPI] ArcTracker weapon kills failed:', err.message);
    }
